...
PUBLIC_CLASS_MAP = {c: c for c in FINAL_CLASSES}
CAMPUS_CLASS_MAP = {
    "塑料袋": "塑料袋",
    "泡沫箱": "泡沫箱",
}

KEEP_NEGATIVE_IMAGES = False

PUBLIC_DIR = "data/public_yolo"
CAMPUS_DIR = "data/own_labeled"
OUT_DIR = "data/merged"
...
def remap_one_dataset(dataset_dir, class_map, final_classes, out_dir, prefix):
    ...
    for lbl_file in tqdm(lbl_files, desc=f"[{prefix}] 合并数据集"):
...
def main():
    ...
    if os.path.exists(CAMPUS_DIR):
        ...
        print(f"[跳过] 未找到校园自采数据集目录: {CAMPUS_DIR}")
